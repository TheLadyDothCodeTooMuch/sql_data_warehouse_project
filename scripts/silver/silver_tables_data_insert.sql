/********************************************************************************************
    ...
    Description:        
        This stored procedure loads and transforms data from the Bronze layer into the 
        Silver layer of the data warehouse. It performs data cleansing, standardization, 
        and integrity checks across multiple CRM and ERP source tables.

    Functional Overview:
        1. Cleans and loads CRM-related tables:
           - silver.crm_cust_info: Removes duplicates, trims names, and standardizes marital status and gender.
           - silver.crm_prd_info: Creates product category IDs, normalizes product line labels, 
             and calculates version end dates.
           - silver.crm_sales_details: Cleans date fields, recalculates sales amounts, and validates quantity/price.

        2. Cleans and loads ERP-related tables:
           - silver.erp_cust_az12: Standardizes customer IDs, validates birthdates, and normalizes gender values.
           - silver.erp_loc_a101: Cleans location identifiers, standardizes country names.
           - silver.erp_px_cat_g1v2: Loads product category mappings from bronze without transformation.

        3. Measures and prints duration for each table load to monitor performance.

    Error Handling:
        - Implements TRY...CATCH for runtime exception handling.
        - Prints descriptive error messages if any load step fails.

    Notes:
        - Designed for repeatable execution; truncates or recreates tables as necessary.
        - Intended for use in ETL pipelines to maintain a clean, consistent Silver layer.
        - Timestamps in logs provide traceability and performance insights.
